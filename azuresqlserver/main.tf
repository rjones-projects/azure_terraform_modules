...
locals {
  ...
  sqlservername_primary = "sql-${var.sqlserver_name}-primary-${local.location}"
  sqlservername_secondary = "sql-${var.sqlserver_name}-secondary-${local.location}"
}
...
resource "azurerm_mssql_server" "primary" {
  name                         = local.sqlservername_primary
  resource_group_name          = local.resource_group_name
  location                     = local.location
  version                      = "12.0"
  ...
  tags                         = merge({ "Name" = local.sqlservername_primary}, var.tags, )
...
resource "azurerm_mssql_server" "secondary" {
  ...
  name                         = local.sqlservername_secondary
  resource_group_name          = local.resource_group_name
  location                     = var.secondary_sql_server_location
  version                      = "12.0"
  administrator_login          = var.admin_username == null ? "sqladmin" : var.admin_username
  administrator_login_password = var.admin_password == null ? random_password.main.result : var.admin_password
  tags                         = merge({ "Name" = local.sqlservername_secondary}, var.tags, )
