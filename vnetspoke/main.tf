...
resource "azurerm_virtual_network" "vnet" {
  name                = lower("vnet-spoke-${var.spoke_vnet_name}-${local.location}-001")
...
#-------------------------------------------------
# route_table to dirvert traffic through Firewall - must exist 
#-------------------------------------------------
...
resource "azurerm_subnet_route_table_association" "rtassoc" {
  for_each       = var.subnets
  subnet_id      = azurerm_subnet.snet[each.key].id
  route_table_id = data.azurerm_route_table.rtout.id
}
