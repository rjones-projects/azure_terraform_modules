...
#---------------------------------------------------------
# Application Insights resources - Default is "false"
#----------------------------------------------------------
...
#-------------------------------------------------------------
# App Service Private Endpoint - Default is "false"
#-------------------------------------------------------------
resource "azurerm_private_endpoint" "private-endpoint" {
  count               = var.enable_private_endpoint == true ? 1 : 0
  name                = lower(format("pl-%s", var.app_service_name))
  location            = local.location
  resource_group_name = local.resource_group_name
  subnet_id           = var.private_endpoint_subnet_id
  private_service_connection {
    is_manual_connection           = false
    name                           = lower(format("pc-%s", var.app_service_name))
