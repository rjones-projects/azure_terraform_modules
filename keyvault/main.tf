...
resource "azurerm_key_vault" "main" {
  ...
  tags                            = merge({"ResourceName" = local.keyvault_name}, var.tags)
...
resource "azurerm_private_endpoint" "pep1" {
  ...
  tags                = merge({ "Name" = format("pe-%s", local.keyvault_name) }, var.tags )
...
resource "azurerm_private_dns_zone_virtual_network_link" "vent-link1" {
  ...
  resource_group_name   = var.existing_private_dns_zone_resource_group
